...
We compare first-order and second-order mean and covariance 
propagation against Monte Carlo ground truth for 
$SE(3)$ composition with varying noise levels $\sigma$.
The Monte Carlo reference is computed from $10^4$ samples 
of the composed pose $h = g \cdot f$ where both $g$ and $f$ 
carry independent Gaussian perturbations in exponential coordinates.

For moderate noise ($\sigma \approx 0.1$~rad in rotation), 
the first-order propagation~\eqref{eq:firstordercomp} is 
accurate to within a few percent. As $\sigma$ increases, 
the second-order corrections from 
Theorem~\ref{thm:meanshift} and Theorem~\ref{thm:covcorrection} 
reduce the mean error by an order of magnitude and bring the 
covariance Frobenius norm error below $1\%$ for 
$\sigma \leq 0.3$~rad.

% [TODO: Generate quantitative figure from Rust implementation]
...
\beq
   U = q_0\,\mathsf{u}_0 + q_i\,\mathsf{u}_i\,,
