...
\beq
    f \cong (\mathsf{R}(\bfOmega_f), \mathbf{T}_f),\;
...
\beqar
    ...
    \mathbf{T} &=& \mathsf{S}(\bfOmega)\, \mathbf{t},\;
...
$\mathcal{O}(\sigma^4)$ from the curvature of $SE(3)$:
\beq
    \sfSigma{hh}^{(2)} = \sfSigma{hh}^{(1)} + 
    \Delta\Sigma^{(2)},
\eeq
where $\sfSigma{hh}^{(1)}$ is the first-order 
result~\eqref{eq:firstordercomp} and $\Delta\Sigma^{(2)}$ 
depends on the fourth central moments of the perturbations 
and the curvature tensor of $SE(3)$ at the base point.
...
3D localization scenario with $N = 3$ landmarks 
in $[-1,1]^3$, camera at $\mathbf{T}_{f_0} = [0,0,-10]$, 
and additive Gaussian noise $\sigma = 1.0$ on landmarks.

% [Figure: side-by-side scatter plots in L2 vs L1 coordinates]
% Reference to original Figs 1,2 with updated formatting

The sample distribution in second-kind coordinates 
$[\bfOmega, \mathbf{T}]$ shows strong translational 
bias, while exponential coordinates 
$[\bfOmega, \mathbf{t}]$ yield a nearly unbiased, 
approximately Gaussian distribution centered at the origin.

\subsection{Experiment 2: Second-Order Propagation Accuracy}

We compare first-order and second-order mean/covariance 
propagation against Monte Carlo ground truth for 
$SE(3)$ composition with varying noise levels $\sigma$.

% [Figure: mean error and covariance Frobenius norm error
%  vs sigma for first-order, second-order, and MC truth]

\subsection{Experiment 3: Saddlepoint vs.\ Laplace Marginalization}

Using natural terrain stereo image pairs with 68 matched 
feature points and $\sigma = 0.8\%$ of image dimensions:

% [Figure: convergence comparison - BA vs Laplace-marginal 
%  vs Saddlepoint-marginal, log-likelihood vs iterations]

% [Figure: pose estimate sample distributions comparing
%  F-matrix, Laplace-marginal, and Saddlepoint-marginal]

The saddlepoint-marginalized MAP estimator achieves:
(a)~convergence basin comparable to or larger than 
Laplace marginalization;
(b)~reduced systematic bias in pose estimates;
(c)~operations on matrices of size $6 \times 6$ 
versus $210 \times 210$ for joint BA.
...
\beq
   U = q_0\,\mathsf{u}_0 + q_i\,\mathsf{u}_i\,,\quad
   q_0 = \cos\frac{\Theta}{2}\,,\quad
   {\bf q} = \sin\frac{\Theta}{2}\;{\bf r}\,,\quad
...
a vector $\bfomega_b$ with $\cos\vartheta>0$ produces a result in the direction $-\bfOmega_a$, 
while $\cos\vartheta<0$ leaves the resulting vector unchanged.
...
\beq
    ...
    \Delta\bfomega, \, u(x)=\int_0^1 e^{-s x} \; ds
    \label{eq:weinorman}
\eeq
...
The second-order covariance correction arises from:
(i)~the quadratic terms in the BCH expansion contributing 
to the variance of $\bfOmega_c$, and
(ii)~the coupling Jacobian $\mathsf{J}_t$ contributing 
cross-terms between rotation and translation.

The correction takes the form
\beq
    \Delta\Sigma^{(2)}_{ij} = \frac{1}{4}\sum_{\alpha\beta\gamma\delta}
    \varepsilon_{i\alpha\beta}\,\varepsilon_{j\gamma\delta}\,
    \left(\langle \delta_\alpha \delta_\beta \delta_\gamma \delta_\delta \rangle
    - \langle \delta_\alpha \delta_\beta \rangle \langle \delta_\gamma \delta_\delta \rangle\right),
    \label{eq:covcorrection_explicit}
\eeq
where $\delta_\alpha$ are the perturbation components and the fourth-order
cumulant captures the non-Gaussian contribution from the manifold curvature.
For Gaussian perturbations, this simplifies via Isserlis' theorem to 
products of second moments weighted by structure constants.
