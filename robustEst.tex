...
defined $[{\bfomega},{\bf v}]$ as the concatation of 2 vectors.},
...
\eeq
where $g$ in the exponent of (\ref{eq:conjugation}) is interpreted as a 4x4 matrix, according to
...
This implies that a change in the normal vector direction can be compensated by a $2\pi$ offset, $({\bf r},\Theta) \cong (-{\bf r},2\pi-\Theta)$. Hence, $\bfomega$ uniquely specifies a rotation only when it is restricted within a sphere of radius $\pi$. Penetrating the skin of this sphere causes {\em phase reflection} and constitutes some difficulty in the inversion process, see below. This fact is also well known as double connectedness,
$SO(3) \cong S^3/Z_2$, where opposite points on the skin of the sphere are identical with each other,
...
unique at the phase reflection boundary, $\Theta = \pi$. The existance of such boundary will have
implications for the topics considered in the following sections.

\subsection{Baker-Campbell-Hausdorff formula, {\em adding} two finite rotations}
...
\beqar
    ...
    \ldots\; . \label{eq:BCH}
\eeqar
However, (\ref{eq:BCH}) only hands an infinite series expansion. It is quite useful to have a closed expression for the Rodrigues vector $\bfOmega_c$ of the resulting rotation ${\sf R}_c = \exp( \hat{\bf H}\cdot\bfOmega_c )$. We could compactly denote this operation as $\bfOmega_c = \ln {\sf R}_a{\sf R}_b\,$. One can show, i.~e. by using a quaternion representation
\cite{myself}, that the following Eqn.~holds,
\begin{multline}
...
\eeq
\end{enumerate}
\subsubsection{phase reflection $\Theta=\pi$}
Since the logarithmic map loses its uniqueness at $\Theta=\pi$, we have to be careful to define the
...
be used to {\em add} finite Rodrigues vectors without employing intermittant rotation matrices or quaternions!
...
constitutes an algebraic vectors space with the normal commutative addition. More exactly, the
vector space is known as $T_1SO(3)$.

\subsection{Wei-Norman formula, derivative of exponentials, {\em adding} finite to infinitesimal rotation}
\label{app:weinorman}
...
mapping from the tangent space at at ${\sf R}(\bfOmega_a)$ to the one at identity.
In order to evaluate the left hand side of (\ref{eq:directionalderiv}), it is advantagous to
...
to obtain a closed form expression from (\ref{eq:weinorman}). It shall be noticed that with Eq.~(\ref{eq:addSO3finite})
...
\eeq
Further, Eq.~(\ref{eq:jacobianomega1}) can be evaluated as,
\beq
...
\eeq
It shall be emphazised that the Jacobiens above don't have singularities and are invertible, since all quantities
are well defined for $\Theta\leq\pi$ and $\mathrm{det}( {\sf J}_{\omega_r}) = (1-x)^2 + \Theta_a^2/4$.

\subsection{Vector {\em addition} formulas over $SE(3)$}
In close analogy to the previous section, a composition in $SE(3)$ can be analyzed
in the {\em additive} framework,
\beq
    ({\sf R}(\bfOmega_c),{\bf T}_c)=({\sf R}_a,{\bf T}_a)\cdot
    ({\sf R}_b,{\bf T}_b)\, .
\eeq
\subsubsection{{\em adding} finite motions:}
...
\eeq
\subsubsection{{\em adding} finite to infinitesimal motion}
...
\beq
    ...
    {\sf J}_{\omega_r}(\bfOmega_a),
\eeq
A more detailed calculation for the coupling Jacobien ${\sf J}_{t_r}$ yields
\beq
...
\eeqar
with $ t_a = {\bf r}_a\cdot{\bf t}_a$, which finishes the discourse on the $SE(3)$ motion group.
...
The $SO(3)$ generating algebra ${\sf H}_i$ and the $SU(2)$ one with generators ${\sf u}_i$ have very similar structures,
...
\end{IEEEeqnarray*}
Other representations of $\mathfrak{su}(2)$ are possible and standard in Physics with changing the
algebra-structure (\ref{eq:su2-algebra-structure}) slightly to $[{\sigma}_i,{\sigma}_j]_c = 2i\varepsilon_{ijk}{\sigma}_k$,
welcome the Pauli-matrices here:
\beq
...
$SU(2)$ is not isomorphic to $SO(3)$, as can be seen from these formulas:
\beq
    R() = whatever
\eeq
