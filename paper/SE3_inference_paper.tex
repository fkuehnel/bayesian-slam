...
This paper does not propose a new basic calculus for Lie groups;
rather, it develops a higher-order probabilistic inference framework
on top of the standard $SE(3)$ machinery used in robotics.
Its contributions are fourfold:

\begin{itemize}
\item A bias analysis of pose parametrization showing that
Lie-Cartan coordinates of the first kind provide the natural
centered coordinates for Gaussian perturbations on $SE(3)$,
whereas second-kind translational coordinates inherit bias
through the nonlinear rotation--translation coupling.

\item Closed-form first- and second-order propagation formulas
for uncertain pose composition and point transformation on
the full $SE(3)$ group, including the coupling Jacobian
$\mathsf{J}_t(\bfOmega,\mathbf{t})$.

\item A saddlepoint marginalization method for eliminating
Euclidean latent variables from projective pose-landmark
posteriors, yielding a more accurate pose marginal than the
standard Laplace/Gauss--Newton approximation.

\item A closed-form finite Rodrigues/BCH composition formula
via the $SU(2)$ double cover, including explicit treatment of
the phase-reflection boundary at $\Theta=\pi$.
\end{itemize}

\subsection{Related Work}

The use of Lie groups in robotics and estimation is by now
well established. Pennec and Thirion~\cite{pennec97}
introduced compositional uncertainty models on $SE(3)$.
The invariant EKF literature, including Barrau and Bonnabel,
showed how group-affine system structure yields autonomous
error dynamics and improved consistency properties.
Sol\`a et al.~\cite{sola2018micro} and Barfoot~\cite{barfoot2024}
provided widely used references for the differential calculus of
$SO(3)$ and $SE(3)$, including exponential and logarithmic
maps, adjoints, Jacobians, and covariance transport.
Forster et al.~\cite{forster2017manifold} demonstrated the
practical importance of this framework through manifold-based
IMU preintegration. These works established the modern
first-order calculus on Lie groups that underlies much of
contemporary SLAM, visual odometry, and state estimation.

A second line of work studies uncertainty on Lie groups beyond
local linearization. Chirikjian~\cite{chirikjian2012stochastic}
developed global stochastic models and harmonic-analysis
methods on Lie groups. Wang and Chirikjian~\cite{wang2008nonparametric}
considered nonparametric uncertainty representations, while
Ye and Chirikjian~\cite{ye2024uncertainty} studied propagation
through stochastic differential equations on unimodular groups.
Bourmaud et al.~\cite{bourmaud2013discrete} developed
Lie-group variants of recursive estimators. These works make
clear that uncertainty on nonlinear groups cannot always be
captured adequately by Euclidean first-order approximations,
but they do not provide the closed-form higher-order
$SE(3)$ propagation formulas developed here for the specific
right-compositional Gaussian model used in robotics.

The present paper also builds on the author's earlier
contributions~\cite{kuehnel2004,kuehnel2005,kuehnel2008}.
In~\cite{kuehnel2004}, Lie-Cartan coordinates of the first kind
and the marginalized MAP viewpoint for Bayesian inference on
$SE(3)$ were introduced. In~\cite{kuehnel2005}, the graphical
structure of SLAM was analyzed using local-frame junction
trees. The present work substantially extends that foundation:
it develops explicit second-order propagation formulas on the
full $SE(3)$ group, establishes the parametrization-bias
distinction between first- and second-kind coordinates,
derives a saddlepoint correction for marginalizing Euclidean
latent variables under projective observation models, and
provides a closed-form finite Rodrigues/BCH treatment via
the $SU(2)$ double cover with explicit handling of phase
reflection at $\Theta=\pi$.

The novelty of this paper is therefore not a new basic Lie
theory for $SE(3)$ itself, but a new higher-order inference
framework built on top of standard Lie-group foundations.
Specifically, the paper contributes:
(i) closed-form first- and second-order propagation of mean
and covariance for uncertain compositions on $SE(3)$,
including the rotation-translation coupling Jacobian;
(ii) a bias analysis showing why first-kind exponential
coordinates provide the natural centered parametrization for
Gaussian perturbations, whereas second-kind translational
coordinates inherit systematic bias through the nonlinear
coupling map; and
(iii) a saddlepoint marginalization scheme for projective
pose-landmark models that improves on the standard
Laplace/Schur approximation while preserving the reduced
pose-only formulation.
...
to obtain an expression for $\mathsf{S}^{-1}(\bfOmega)$. As an interesting fact, we meet the Rodrigues Jacobians again
\begin{multline}
    \mathsf{S}^{-1}(\bfOmega) = (1-x)\mathsf{I} + x\, {\bf r}{\bf r}^T - {1\over 2}[\bfOmega]_\times \\
    = \mathsf{J}_{\omega_r}(-\bfOmega) = \mathsf{J}_{\omega_l}(\bfOmega)\;.
    \label{eq:Sinvidentity}
\end{multline}
Since $\mathsf{S}^{-1} = \mathsf{J}_{\omega_l}$, the determinant is $\det\mathsf{J}_{\omega_l} = (1-x)^2 + \Theta^2/4$ and hence $\det\mathsf{S} = 2(1-\cos\Theta)/\Theta^2$, confirming invertibility for all $\Theta \in [0,\pi]$.
...
is taken with the \emph{physical translation} ${\bf T}$ held fixed
and only $\bfOmega$ varying in $\mathsf{S}^{-1}$.
Differentiating $\mathsf{S}^{-1}(\bfOmega)$ with respect to $\bfOmega$
and applying to the fixed vector ${\bf T}$ yields:
\begin{eqnarray}
    \frac{\partial[\mathsf{S}^{-1}{\bf T}]}{\partial\bfOmega} &=&
    \frac{1}{2}[{\bf T}]_\times + \frac{x}{\Theta}\,{\bf r}{\bf T}^T
    + \alpha\,{\bf T}{\bf r}^T + \nonumber\\
    && \bar T\!\left[\frac{x}{\Theta}\mathsf{I}
    - \left(\frac{2x}{\Theta}+\alpha\right){\bf r}{\bf r}^T\right] \nonumber\\
    &=& {1\over 2}[{\bf t}]_\times+ {x\over\Theta}
    \left( {\bf r}{\bf t}^T- {\bar t} \;\mathsf{H}^2 + {\bf t}{\bf r}^T \right) + \nonumber \\
    && {{\bar t}\over\Theta} (\mathsf{S}(\bfOmega)-\mathsf{S}^{-1}(-\bfOmega)),
    \label{eq:dSinvTdOmega}
\end{eqnarray}
where $\bar t = {\bf r}\cdot{\bf t}$ and $\bar T = {\bf r}\cdot{\bf T}$ is the axial component
of ${\bf T}$ and
\beq
    \alpha = \frac{\sin\Theta - \Theta}{2(1-\cos\Theta)}
    = -\frac{1}{2}\cot\frac{\Theta}{2} + \frac{1}{\Theta}\;.
    \label{eq:alphadef}
\eeq
