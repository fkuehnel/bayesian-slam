...
Monte Carlo validation confirms a $1.8\times$ improvement 
in covariance accuracy at $\sigma_\omega \approx 0.2$~rad,
with the improvement scaling as $\sigma_\omega^4$.

\item A saddlepoint marginalization scheme~\eqref{eq:saddlebody}
that eliminates latent Euclidean variables from the joint 
posterior more accurately than the standard Laplace 
(Gauss-Newton) approximation.
The correction involves two distinct contraction types
of the third derivatives (cross and trace) plus a 
quartic term, and improves the marginal integral by 
up to $\sim 3000\times$ over Laplace in the perturbative 
regime ($\sigma_{\rm depth}/x_3' \lesssim 0.3$).
...
Numerical verification confirms machine-precision
accuracy across all angle regimes, with quantified
truncation orders for the BCH series
(Table~\ref{tab:bchorder}).
\end{enumerate}

All formulas have been verified symbolically in
Mathematica and numerically against finite differences
and Monte Carlo references. The framework is applicable 
beyond pose estimation to any domain requiring 
probabilistic inference on $SE(3)$: IMU preintegration, 
spacecraft attitude determination, surgical robot 
registration, and multi-body dynamics.
...
manifolds; (iv)~deriving a closed-form expression
for $\partial[\mathsf{S}^{-1}\mathbf{T}]/\partial\bfOmega$
to replace the current numerical differentiation
in the coupling Jacobian.
