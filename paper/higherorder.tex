...
\end{multline}
Since $\mathsf{S}^{-1} = \mathsf{J}_{\omega_l}$, the determinant
of $\mathsf{S}^{-1}$ is $\det\mathsf{J}_{\omega_l} = (1-x)^2 + \Theta^2/4$,
confirming invertibility for all $\Theta \in [0,\pi]$. This is
consistent with~(\ref{eq:detS}): $\det\mathsf{S} = 2(1-\cos\Theta)/\Theta^2$
and $\det\mathsf{S}^{-1} = \Theta^2/(2(1-\cos\Theta))$.
...
\eeq
The coupling Jacobian $\mathsf{J}_{t_r}$ is defined by
\beq
